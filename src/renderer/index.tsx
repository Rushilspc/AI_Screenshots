...
function App(): JSX.Element {
  ...
  async function triggerCapture(mode: CaptureMode): Promise<void> {
    setStatus(`Queued ${mode} capture...`);
    await window.aiScreenshots.capture(mode, "high");
    await refreshStats();
  }

  async function importFolder(): Promise<void> {
    const count = await window.aiScreenshots.importFolder();
    if (count > 0) {
      setStatus(`Imported ${count} files. Processing in background...`);
    } else {
      setStatus("Import canceled or no images found.");
    }
    await refreshStats();
  }
  ...
  return (
    <main className="page">
      ...
      <section className="actions">
        ...
        <button onClick={() => void importFolder()}>Import Folder</button>
      </section>

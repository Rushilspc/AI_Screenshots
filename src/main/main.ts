  ...
  const menu = Menu.buildFromTemplate([
    ...
    { label: "Capture Fullscreen", click: () => enqueueCapture("fullscreen") },
    {
      ...
      click: async () => {
        ...
        await importFromDialog(mainWindow);
      }
    },
...
function scheduleAutoImport(): void {
  enqueueImportFiles(captureService.getAutoImportCandidates(), "low");
}

function enqueueImportFiles(files: string[], priority: CaptureJob["priority"]): number {
...
async function importFromDialog(parentWindow: BrowserWindow): Promise<number> {
  ...
  const files = captureService.getImportCandidatesFromFolder(folder);
  return enqueueImportFiles(files, "normal");
}
...
function wireIpc(): void {
  ...
  ipcMain.handle("import:folder", async () => {
    ...
    return importFromDialog(mainWindow);
  });

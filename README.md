...
Generated installer files will be in `release/`.

## Publish `.exe` on GitHub (recommended)

Do **not** commit `.exe` binaries directly into normal git history. Use GitHub Actions artifacts or Releases instead.

### Option A: One-click manual build artifact

1. Push your branch to GitHub.
2. Go to **Actions → Build Windows EXE → Run workflow**.
3. Download `ai-screenshots-windows-exe` artifact.
4. Share that `.exe` with users.

### Option B: Tagged release (best for users)

1. Create and push a version tag, e.g. `v0.1.0`.
2. The workflow builds the installer and attaches `.exe` files to the GitHub Release.
3. Users download the `.exe` from the Release page and run it.

## Install and run on your computer

1. Download the installer `.exe` from artifact/release.
2. Double-click it and complete setup.
3. Launch **AI Screenshots** from Start Menu.
4. The app minimizes to tray when you close the window.
